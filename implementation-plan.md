...
1. Settings foundation: create **`@vue-html-bridge/settings`** (settings.md) — the schema and defaults, `mergeSettings` / `validateSettings` / `decomposeSettings`, the workspace-file loader, and the `schema.json` golden — and wire the language server to it (language-server.md §9.2). Unknown-field warning, invalid-type safe fallback; runtime validation of `validators[].settings` per the Phase 1 decision (must be in place before Phase 3 exposes external adapters). Settings must drive behavior, not merely parse: explicit tasks and tests for `enabled`, `include`/`exclude` document filtering, and the `validateOnChange` / `validateOnSave` scheduling gates. Building the schema in the shared package now is what lets the Phase 3 CLI reuse it unchanged.
...
3. **External adapter loading + trust** (language-server §10.2): package-specifier allowlist rules, workspace resolution, runtime-check + `apiVersion` gate, per-adapter failure isolation with deduped notices. Verify that an external adapter's concrete watch targets trigger only that adapter's session recreation. ADRs: package specifier/PnP support; sandboxing feasibility conclusion (monorepo.md §15).
...
5. **`@vue-html-bridge/cli`**: grow the Phase 2 internal runner into the published CLI (cli.md) — the full §4.2 flag surface with settings parity, per-adapter validator flags (`--validator` / `--disable-validator` / `--validator-setting`), text/JSON output goldens, exit codes and `--fail-on`, SIGINT abort, trust behavior (invocation-trusted default, `--untrusted`), and the same external-adapter gates as the language server. Gate: the CLI/LSP parity E2E (cli.md §9 item 12; monorepo.md §12.2).
6. **Release engineering**: Changesets (per ADR-0001) with independent versions; `schema.json` generated from `@vue-html-bridge/settings` and shipped by language-server; README/usage docs per package; license/provenance checks; the client-launcher decision (language-server §14, "decided at Phase 3's initial release"). Publish verification for every public package, not just the adapter path: `pnpm pack` each package, install the tarballs into an out-of-repo smoke project, import and run them — for language-server, launch `dist/bin.js --stdio` and complete an initialize handshake; for the CLI, run `vue-html-bridge` against a fixture workspace and check its output and exit code.
...
| — | CLI follow-ups: watch mode, SARIF/other output formats, stdin input, persistent cross-run cache, file-level parallel analyze (cli.md §10) | After the initial release |
...
| Tests | Lands in |
| --- | --- |
| 1–7 | Phase 2 Track 4 (the package is created with the language-server settings work) |

### cli.md §9

| Tests | Lands in |
| --- | --- |
| 1–12 | Phase 3 task 5 (the Phase 2 internal runner is unpublished and carries no public test surface) |
