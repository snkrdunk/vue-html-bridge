...
| 5 | Delivery form | A separate adapter library per linter (for example, vue-html-bridge-markuplint), each implementing its own reverse mapping | An LSP language server as the main delivery form. Validators are adapters that plug into a shared SPI (validator-api), and the bridge side runs them. The initial release also ships a one-shot CLI (`@vue-html-bridge/cli`) as a second consumer of the analyzer, with settings parity to the language server through the shared `@vue-html-bridge/settings` package (monorepo.md §1, §14; cli.md). It was out of scope in earlier revisions of this directory's design; 2026-08-20 brought it into the initial release | The product goal is immediate feedback while editing, plus the same analysis in CI. Because the analyzer's API does not depend on the LSP, the CLI is a thin second consumer, not a second implementation |
